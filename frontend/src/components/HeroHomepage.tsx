...
const STATUTE_MODES = [
  { id: "axis-2.0", label: "Axis 2.0 • Statutes", desc: "Indian Criminal & Special statutory frameworks" },
  { id: "bns", label: "BNS / BNSS / BSA 2023", desc: "Bharatiya Nyaya & Nagarik Suraksha Sanhitas" },
  { id: "pocso", label: "POCSO Act 2012", desc: "Child sexual offence provisions & mandatory rules" },
  { id: "ndps", label: "NDPS Act 1985", desc: "Search, seizure, contraband, and bail sections" },
  { id: "all", label: "All Indian Codes", desc: "Comprehensive legal citation and acts browser" },
];

const COMPASS_GUIDANCE_MODES = [
  { id: "guidance", label: "Compass • Guidance", desc: "Step-by-step procedural direction & legal routing" },
  { id: "jurisdiction", label: "District Court Jurisdiction", desc: "Local territorial & pecuniary court competencies" },
  { id: "police", label: "Police & FIR Procedure", desc: "Cognizable complaints, Zero FIR & statutory timeframes" },
  { id: "special", label: "Special Courts (POCSO/NDPS)", desc: "Fast-track designated sessions & bail tribunals" },
  { id: "hierarchy", label: "Appellate Hierarchy", desc: "High Court petitions, appeals & writ remedies" },
];
...
export const HeroHomepage: React.FC<HeroHomepageProps> = ({
  ...
  const [activeFilter, setActiveFilter] = useState<"bare-act" | "judgement" | null>(null);
  ...
  useEffect(() => {
    setSelectedMode(isCompass ? COMPASS_GUIDANCE_MODES[0].id : STATUTE_MODES[0].id);
  }, [isCompass]);
    ...
    <div className="relative min-h-full w-full flex flex-col justify-between items-center select-none overflow-x-hidden p-4 sm:p-6 md:p-8 text-[#F5F5F0]">
          ...
          {isCompass ? (
            <div className="flex flex-col items-center gap-2 mt-2">
              ...
              <p className="text-xs sm:text-sm text-zinc-400 max-w-xl mx-auto font-sans leading-relaxed tracking-normal italic">
                Find your bearings in the legal system — one clear step at a time.
              </p>
            </div>
          ) : (
            <p className="text-xs sm:text-sm text-zinc-400 max-w-xl mx-auto font-sans leading-relaxed tracking-normal">
              Project Access surfaces the applicable Indian laws, sections, acts, and precedents for you to read and understand — it does not offer legal advice.
            </p>
          )}
        </motion.div>
            ...
            <div className="mt-3 pt-3 border-t border-white/5 flex items-center justify-between gap-2 flex-wrap">
              ...
              <div className="flex items-center gap-2 flex-wrap">
                ...
                <button
                  ...
                  onClick={() => alert("Attach FIR copy, lease agreement, or statutory legal notice.")}
                        ...
                        return (
                          ...
                          >
                            ...
                            <div className="min-w-0">
                              ...
                              <div className="text-[10px] text-zinc-400 leading-snug mt-0.5">{mode.desc}</div>
                            </div>
                          </button>
                        );
                      })}
                ...
                <button
                  type="button"
                  onClick={() => setActiveFilter(activeFilter === "bare-act" ? null : "bare-act")}
                ...
                <button
                  type="button"
                  onClick={() => setActiveFilter(activeFilter === "judgement" ? null : "judgement")}
                  className={`btn-glass btn-glass-pill px-3 py-1.5 flex items-center gap-1.5 text-xs transition-all ${
              ...
              <button
                ...
                title={isGenerating ? "Stop retrieval" : "Submit situation (Enter)"}
            ...
            {[
              {
                ...
                query: "Landlord refusing to refund security deposit after vacating flat. What Indian legal provisions, criminal breach of trust sections, and remedies apply?",
              },
              {
                title: "Employer withholding salary",
                query: "Company withholding salary for 3 months without notice. What labor laws, Payment of Wages Act, and statutory sections apply?",
              },
              {
                title: "Dowry harassment complaint",
                query: "Dowry demand and harassment by husband and in-laws. What statutory sections under BNS 85/86 and Domestic Violence Act apply?",
              },
            ].map((item, idx) => (
              <button
                key={idx}
      ...
      <footer className="w-full flex items-center justify-between z-10 pb-2 pt-2 px-1 sm:px-4">
        ...
        <div className="shrink-0">
          <button
            type="button"
            onClick={() => alert("Cookie preferences: Essential analytical and statutory session cookies enabled.")}
