  ...
  return (
    <div className="flex items-start gap-3 group">
      ...
      <img
        src="/icon.jpeg"
        alt="Project Access"
        className="w-7 h-7 rounded-lg object-cover flex-shrink-0 mt-1 border border-zinc-200 dark:border-zinc-800"
      />

      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2 text-[11px] text-zinc-400 mb-1">
          <span className="font-medium text-zinc-700 dark:text-zinc-300">Project Access</span>
          <span>•</span>
          <span>{timeStr}</span>
        </div>
        ...
        {message.sources && message.sources.length > 0 && (
          <div className="mt-3 pt-2 border-t border-zinc-200/60 dark:border-zinc-800/60">
            <div className="text-[11px] font-medium text-zinc-400 mb-1.5">Sources & Statutory Citations:</div>
            <div className="flex flex-wrap gap-1.5">
              {message.sources.map((src, i) => (
                ...
                >
                  ...
                  {src.section_number && (
                    <span className="text-zinc-400">({src.section_number})</span>
                  )}
                </div>
              ))}
            </div>
